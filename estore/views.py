...
def add_to_cart(request):
    ...
    if cart_total()+product.price < 50000:
        ...
        messages.error(request, 'Couldn\'t add item to the cart because orders worth more than $50,000 can\'t be created.')
...
def increment_cart(request, cart_id):
    if request.method == 'GET':
        ...
        if cart_total()+cart_item.product.price < 50000:
            ...
            messages.error(request, 'Couldn\'t increment the quantity because orders worth more than $50,000 can\'t be created.')
    return redirect('estore:cart')
